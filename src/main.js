...
app.listen(port, () => log.info({port}, "Starting Ather Collector"))
...
process.on("uncaughtException", error => {
  log.error({error: errorFormatter(error)}, "Got an uncaught exception")
  delayAndExit(2)
})
process.on("unhandledRejection", error => {
  log.error({error: errorFormatter(error)}, "Got unhandled promise rejection")
